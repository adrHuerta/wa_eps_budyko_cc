...
# for analysis
piscotx_mean_anual = piscotx_anual.sel(time=slice("1982-01-01", "2011-12-31")).mean(dim="time")
piscotx_mean_anual = piscotx_mean_anual.reindex(longitude=piscopet.longitude.values,
                                                latitude=piscopet.latitude.values,
                                                method="nearest")
np.round(piscotx_mean_anual, 1).to_netcdf("data/processed/present/PISCO/temp/TX_mean_anual.nc")
