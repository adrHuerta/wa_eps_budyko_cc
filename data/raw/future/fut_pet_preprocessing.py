...
for nfile in range(len(nc_files_tmax)):
    ...
    for year in range(2035, 2066):
        ...
        xr.apply_ufunc(hargreaves_samani,  # hargreaves_samani(time_i, tmax_i, tmin_i, lat_i)
            ...
            to_dataset(name="pet"). \
            to_netcdf("/content/drive/MyDrive/Google_Colab_temp/others/SNS/pet/pet_" + nc_files_tmax[nfile].split("/")[
            -1] + "_" + str(year) + ".nc", encoding=encoding, engine='netcdf4')

    piscopet = xr.open_dataset(cdo.cat(input=sorted(glob.glob(
        "/content/drive/MyDrive/Google_Colab_temp/others/SNS/pet/pet_" + nc_files_tmax[nfile].split("/")[
            -1] + "*.nc"))))
    ...
    piscopet_anual = piscopet_anual.reindex(longitude=PISCOsns_grid.longitude.values,
                                            ...
                                            method="nearest")
    piscopet_anual.to_netcdf(
        "/content/drive/MyDrive/Google_Colab_temp/others/SNS/pet_" + nc_files_tmax[nfile].split("/")[-1],
