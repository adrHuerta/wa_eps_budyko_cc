...
for nc_file in nc_files_tmax:
    ...
    pisco_tx_anual = pisco_tx.resample(time="1Y").mean().mean(dim="time")  # 30 years
    pisco_tx_anual = pisco_tx_anual.reindex(longitude=PISCOsns_grid.longitude.values,
                                            latitude=PISCOsns_grid.latitude.values,
                                            method="nearest")
    pisco_tx_anual.to_netcdf("/content/drive/MyDrive/Google_Colab_temp/others/SNS/tx_" + nc_file.split("/")[-1],
                             encoding=encoding_tx, engine='netcdf4')
...
for nc_file in nc_files_tmin:
    ...
    pisco_tn_anual = pisco_tn.resample(time="1Y").mean().mean(dim="time")  # 30 years
    pisco_tn_anual = pisco_tn_anual.reindex(longitude=PISCOsns_grid.longitude.values,
                                            latitude=PISCOsns_grid.latitude.values,
                                            method="nearest")
    pisco_tn_anual.to_netcdf("/content/drive/MyDrive/Google_Colab_temp/others/SNS/tn_" + nc_file.split("/")[-1],
                             encoding=encoding_tn, engine='netcdf4')